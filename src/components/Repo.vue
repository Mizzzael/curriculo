<template>
    <section
        class="repo w-full py-4 flex flex-wrap items-center justify-center"
    >
        <header class="w-full">
            <a
                :href="repo.html_url"
        ...
        <section class="w-full px-4 flex flex-wrap items-start justify-center">
            ...
            <section class="md:w-4/12 w-full py-1 flex-wrap flex items-center">
                <div class="md:w-full w-6/12">
                    <p
                        v-if="repo.language"
                        class="md:text-md text-sm text-gray-500"
                    >
                        <i
                            :style="
                                `color: ${setColorLanguage(repo.language)};`
                            "
                            class="fa fa-circle"
                        ></i>
                        <span class="md:text-sm text-2xs px-1">{{
                            repo.language
                        }}</span>
                    </p>
                </div>
